...
# county = 'Tharaka'
outfile = 'ndvi_residuals_'+county+'_V2.csv'
...
with pd.read_csv(infile, chunksize=10000) as reader:
    ...
    for ndvi_df in reader:
        ...
        # ndvi_df = pd.read_csv(infile, chunksize= 1000) #change to 10,000 for server
        ...
        #Save to CSV - don't write indices since they wont make sense with chunksize
        #if the file already exists do not re-write the header 
        ndvi_res.to_csv(outfile, mode= 'a',  encoding='utf-8', index=False, header=not os.path.exists(outfile))
        count += 1
